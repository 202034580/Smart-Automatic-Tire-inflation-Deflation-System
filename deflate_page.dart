// ===============================
// deflate_page.dart (FULL UPDATED)
...
class DeflatePage extends StatefulWidget {
  const DeflatePage({
    super.key,
    required this.tire,
    this.device,
  ...
  @override
  State<DeflatePage> createState() => _DeflatePageState();
}

class _DeflatePageState extends State<DeflatePage> {
  ...
  void initState() {
    ...
    _min = kMinPsi;
    _max = psi.floorToDouble().clamp(kMinPsi, kMaxPsi);
    _target = (widget.tire.targetPsi < psi ? widget.tire.targetPsi : _max)
        .clamp(_min, _max);
  ...
  Future<void> _setupBle(BluetoothDevice d) async {
    ...
    try {
      ...
      if (mounted) setState(() => _busyBle = false);
      debugPrint("DeflatePage BLE setup failed: $e");
    }
  }
  ...
  Future<bool> _confirmApply() async {
    ...
    return await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        title: const Text('Confirm deflate'),
        content: Column(
          ...
          children: [
            ...
            Text(
              'Proceed to start deflating? (No auto-stop at 10 PSI)',
  ...
  Future<void> _sendBleDeflateCommand() async {
    final device = widget.device;
    if (device == null) {
      debugPrint('DeflatePage: device is null');
      return;
    }

    // Ensure TX exists (in case setup failed)
    if (_tx == null) {
      await _setupBle(device);
    }
    if (_tx == null) {
      debugPrint('DeflatePage: no TX characteristic found');
      return;
    }

    final tireNum = _tireNumFromName(widget.tire.name);
    final cmd = 'T$tireNum:D\n'; // ✅ matches Arduino handleCommand()
    ...
    try {
      ...
      debugPrint('DeflatePage: sent "$cmd"');
    } catch (e) {
      debugPrint('DeflatePage BLE send error: $e');
    } finally {
      if (mounted) setState(() => _busyBle = false);
    }
  }
  ...
  Future<void> _sendBleStopCommand() async {
    final device = widget.device;
    if (device == null) {
      debugPrint('DeflatePage Stop: device is null');
      return;
    }
    ...
    if (_tx == null) {
      debugPrint('DeflatePage Stop: no TX characteristic found');
      return;
    }
    ...
    try {
      ...
      debugPrint('DeflatePage Stop BLE send error: $e');
    } finally {
      if (mounted) setState(() => _busyBle = false);
    }
  }
  ...
  Future<void> _apply() async {
    ...
    // optional guard: if already at min, don’t deflate
    if (widget.tire.psi <= kMinPsi) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Already near minimum.')),
      );
      return;
    }
    ...
    await _sendBleDeflateCommand();

    HistoryStore.logDeflate();

    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          'Deflate ${widget.tire.name} confirmed (manual stop required)',
        ),
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: kKFUPMGreen,
        foregroundColor: Colors.white,
        title: const Text('Deflate', style: TextStyle(fontWeight: FontWeight.w800)),
        centerTitle: true,
        actions: [
          Padding(
                    ...
                    children: [
                      Expanded(
                        child: ElevatedButton.icon(
                          ...
                          label: const Text('Apply & Deflate'),
